...
namespace Fred
{
  public class Activities
  {
    ...
    // run-time parameters
    bool Enable_default_sick_behavior = false;
    double Default_sick_day_prob = 0.0;
    //double SLA_mean_sick_days_absent = 0.0;
    //double SLU_mean_sick_days_absent = 0.0;
    double SLA_absent_prob = 0.0;
    double SLU_absent_prob = 0.0;
    double Flu_days = 0.0;
    double Hospitalization_visit_housemate_prob = 0.0;
    int HAZEL_seek_hc_ramp_up_days = 0;
    double HAZEL_seek_hc_ramp_up_mult = 1.0;

    AgeMap Hospitalization_prob = null;
    AgeMap Outpatient_healthcare_prob = null;

    ActivitiesTrackingData Tracking_data;

    // Childhood Presenteeism parameters
    double Sim_based_prob_stay_home_not_needed = 0.0;
    double Census_based_prob_stay_home_not_needed = 0.0;

    // sick leave statistics
    double Standard_sicktime_allocated_per_child = 0.0;

    int Sick_leave_dist_method = 0;
    List<double> WP_size_sl_prob_vec;
    List<double> HH_income_qtile_sl_prob_vec;
    double WP_small_mean_sl_days_available = 0.0;
    double WP_large_mean_sl_days_available = 0.0;
    int WP_size_cutoff_sl_exception = 0;
    double HH_income_qtile_mean_sl_days_available = 0.0;
    ...
    void initialize_static_variables()
    {
      ...
      FRED_STATUS(0, "initialize() entered\n", "");

      int temp_int = 0;
      Params::get_param_from_string("enable_default_sick_behavior", &temp_int);
      Enable_default_sick_behavior = (temp_int == 0 ? false : true);
      Params::get_param_from_string("sick_day_prob", &Default_sick_day_prob);
      Params::get_param_from_string("SLA_absent_prob", &SLA_absent_prob);
      Params::get_param_from_string("SLU_absent_prob", &SLU_absent_prob);
      Params::get_param_from_string("wp_small_mean_sl_days_available", &WP_small_mean_sl_days_available);
      Params::get_param_from_string("wp_large_mean_sl_days_available", &WP_large_mean_sl_days_available);
      Params::get_param_from_string("wp_size_cutoff_sl_exception", &WP_size_cutoff_sl_exception);

      Params::get_param_from_string("flu_days", &Flu_days);
      Params::get_param_from_string("prob_of_visiting_hospitalized_housemate", &Hospitalization_visit_housemate_prob);

      Params::get_param_from_string("sick_leave_dist_method", &Sick_leave_dist_method);

      if (!Enable_default_sick_behavior)
      {
        if (Sick_leave_dist_method == WP_SIZE_DIST)
        {
          Params::get_param_vector((char*)"wp_size_sl_prob_vec", WP_size_sl_prob_vec);
          assert(static_cast<int>(WP_size_sl_prob_vec.size()) == Workplace::get_workplace_size_group_count());
          for (int i = 0; i < Workplace::get_workplace_size_group_count(); ++i)
          {
            Tracking_data.employees_with_sick_leave.push_back(0);
            Tracking_data.employees_without_sick_leave.push_back(0);
            Tracking_data.employees_days_used.push_back(0);
            Tracking_data.employees_taking_day_off.push_back(0);
            Tracking_data.employees_sick_leave_days_used.push_back(0);
            Tracking_data.employees_taking_sick_leave_day_off.push_back(0);
            Tracking_data.employees_sick_days_present.push_back(0);
          }
        }
        else if (Sick_leave_dist_method == HH_INCOME_QTILE_DIST)
        {
          Params::get_param_vector((char*)"hh_income_qtile_sl_prob_vec", HH_income_qtile_sl_prob_vec);
          assert(static_cast<int>(HH_income_qtile_sl_prob_vec.size()) == 4);
          for (int i = 0; i < 4; ++i)
          {
            Tracking_data.employees_with_sick_leave.push_back(0);
            Tracking_data.employees_without_sick_leave.push_back(0);
        ...
        {
          Utils::fred_abort("Invalid sick_leave_dist_method: %d", Sick_leave_dist_method);
        }
      }
      ...
      {
        Params::get_param_from_string("HAZEL_seek_hc_ramp_up_days", &HAZEL_seek_hc_ramp_up_days);
        Params::get_param_from_string("HAZEL_seek_hc_ramp_up_mult", &HAZEL_seek_hc_ramp_up_mult);
      }

      if (Global::Report_Childhood_Presenteeism)
      {

        Params::get_param_from_string("standard_sicktime_allocated_per_child", &Standard_sicktime_allocated_per_child);
        ...
        Sim_based_prob_stay_home_not_needed = static_cast<double>(count_has_school_age_and_unemployed_adult) / static_cast<double>(count_has_school_age);
      }
      is_initialized = true;
    }
  ...
  this.networks.clear();
    ...
    void set_profile(Profile _profile)
    {
  ...
  {
    FRED_VERBOSE(1, "initialize_sick_leave entered\n");
    this.my_sick_days_absent = 0;
    this.my_sick_days_present = 0;
    this.my_sick_leave_decision_has_been_made = false;
    this.my_sick_leave_decision = false;
    ...
    if (!Enable_default_sick_behavior)
    {
      ...
      {
        this.sick_leave_available = (Random::draw_random() < WP_size_sl_prob_vec[index]);
      ...
      {
        this.sick_leave_available = (Random::draw_random() < HH_income_qtile_sl_prob_vec[index]);
        // compute sick days available
        this.sick_days_remaining = HH_income_qtile_mean_sl_days_available + Flu_days;
      }

      FRED_VERBOSE(0, "initialize_sick_leave size_leave_avaliable = %d\n",
                   (this.sick_leave_available ? 1 : 0));
    }
    FRED_VERBOSE(0, "initialize_sick_leave sick_days_remaining = %d\n", this.sick_days_remaining);
  }
  ...
  void assign_initial_profile()
  {
    ...
    // weekend worker
    if ((this.profile == WORKER_PROFILE && Random::draw_random() < 0.2))
    {  // 20% weekend worker
  ...
  void update(int sim_day)
  {
    ...
    FRED_CONDITIONAL_VERBOSE(1, sim_day > 0,
           ...
           static_cast<double>(Tracking_data.daily_sick_days_absent) / static_cast<double>(1 + Tracking_data.daily_sick_days_absent +
           ...
           static_cast<double>(Tracking_data.daily_school_sick_days_absent) /
           static_cast<double>(1 + Tracking_data.daily_school_sick_days_absent + Tracking_data.daily_school_sick_days_present));
  ...
  void update_schedule(int sim_day)
  {
    ...
    else
    {
      ...
      if (Global::Enable_Hospitals && !this.myself.is_hospitalized() && static_cast<Household*>(this.myself.get_household()).has_hospitalized_member())
      {
        ...
        if (hh != null)
        {
          if (this.profile != PRISONER_PROFILE && this.profile != NURSING_HOME_RESIDENT_PROFILE && Random::draw_random() < Hospitalization_visit_housemate_prob)
          {
    ...
    if (this.myself.is_adult())
    {
      ...
      else
      {
          ...
          if (index >= 0 && this.my_sick_leave_decision_has_been_made)
          {
            if (this.is_sick_leave_available() && this.sick_days_remaining > 0.0)
            {
              ...
              { //I have sick leave, but my days are about to run out
                ...
                {
                  stay_home = (Random::draw_random() < SLU_absent_prob);
                  if (stay_home)
                  {
          ...
          { //Sick Leave decision hasn't been made
            if (this.is_sick_leave_available() && this.sick_days_remaining > 0.0)
            {
              ...
              { //I have sick leave but I am running out of days, so need to decide what to do
                ...
                if (Random::draw_random() < this.sick_days_remaining)
                {
                  stay_home = (Random::draw_random() < SLA_absent_prob);
                  if (stay_home)
                  {
                    //Want to make sure that tomorrow I roll against the SLU_absent_prob
                    this.my_sick_leave_decision_has_been_made = false;
        ...
        else
        {
          ...
          stay_home = (Random::draw_random() < Default_sick_day_prob);
        }
      }
    }
    else
      ...
      if (Global::Report_Childhood_Presenteeism)
      {
        ...
        if (it_is_a_schoolday)
        {
          ...
          else
          {
            ...
            {
              List<Person*> inhab_vec = my_hh.get_inhabitants();
              for (List<Person*>::iterator itr = inhab_vec.begin(); itr != inhab_vec.end(); ++itr)
              {
                if ((*itr).is_child())
                {
                  continue;
                ...
                {
                  stay_home = true;
                  (*itr).get_activities().sick_days_remaining -= Standard_sicktime_allocated_per_child;
                  my_hh.set_working_adult_using_sick_leave(true);
                  break;
                }
              }
  ...
  std::string schedule_to_string(int day)
  {
    std::stringstream ss;
  ...
  void end_of_run()
  {
    ...
    {
      double mean_sick_days_used = (Tracking_data.total_employees_taking_sick_leave == 0 ? 0.0 : (Tracking_data.total_employees_days_used / static_cast<double>(Tracking_data.total_employees_taking_sick_leave)));
  ...
  int get_daily_activity_location_id(int p)
  {
    return get_daily_activity_location(p) == null ? -1 : get_daily_activity_location(p).get_id();
  }

  const char* get_daily_activity_location_label(int p)
  {
  ...
  int get_index_of_sick_leave_dist(Person* per)
  {
    if (!Enable_default_sick_behavior)
    {
      if (Sick_leave_dist_method == WP_SIZE_DIST)
