...
namespace Fred
{
  public class Population
  {
    ...
    public Population ()
    {
      ...
      this.VaccineManager = new Vaccine_Manager();
    }
    ...
    public Person AddPerson(
      ...
      Place work,
      DateTime day,
      bool todayIsBirthday)
    {
      var person = new Person(age, sex, race, relationship, house, school, work, day, todayIsBirthday);
      return person;
    }

    public AV_Manager av_manager { get; private set; }

    public Vaccine_Manager VaccineManager { get; private set; }
    ...
    public void setup()
    {
      FredUtils.Status(0, "setup population entered\n", "");

      if (Global.IsVaccinationEnabled)
      {
        this.VaccineManager = new Vaccine_Manager(this);
      }
      else
      {
        this.VaccineManager = new Vaccine_Manager();
      ...
      if (Global.IsAntiviralsEnabled)
      {
        FredUtils.Abort("Sorry, antivirals are not enabled in this version of FRED.");
      ...
      if (Global.Verbose > 0)
      {
        for (int d = 0; d < Global.Diseases.Count; ++d)
        {
          ...
          FredUtils.Status(0, "number of residually immune people for disease %d = %d\n", d, count);
        }
      }
      ...
      FredUtils.Status(0, "population setup finished\n", "");
    }

    Person_Init_Data get_person_init_data(string line,
                  ...
                  bool is_2010_ver1_format)
    {
      ...
      {
        FredUtils.Log(2, "WARNING: person %s -- no workplace found for label = %s\n", pid.label,
         pid.work_label);
    ...
    void parse_lines_from_stream(std::istream &stream, bool is_group_quarters_pop)
    {
      ...
      while (stream.good())
      {
        ...
        else
        {
          ...
          // skip this person
          FredUtils.Log(0, "WARNING: skipping person %s -- %s %s\n", pid.label,
           "no household found for label =", pid.house_label);
        }
        FredUtils.Log(1, "person %d = %s -- house_label %s\n", n, pid.label, pid.house_label);
        n++;
      } // <----- end while loop over stream
      FredUtils.Log(0, "end of stream, persons = %d\n", n);
  ...
  FredUtils.Status(0, "Validating synthetic population identifiers before reading.\n", "");
  string pop_dir = Global.Synthetic_population_directory;
  FredUtils.Status(0, "Using population directory: %s\n", pop_dir);
  FredUtils.Status(0, "FRED defines a \"deme\" to be a local population %s\n%s%s\n",
  ...
  for(int d = 0; d<param_num_demes; ++d) {
    ...
    else
    {
      ...
      {
        FredUtils.Abort("Help! %d %s %d %s %d %s\n", param_num_demes,
    ...
    FredUtils.Status(0, "Split ID string \"%s\" into %d %s using delimiter: \'%c\'\n", pop_id_string,
    ...
    FredUtils.Status(0, "Deme %d comprises %d synthetic population id%s:\n", d, this.demes[d].size(),
    ...
    for (int i = 0; i < this.demes[d].size(); ++i)
    {
      ...
      FredUtils.Status(0, "-. Deme %d, Synth. Pop. ID %d: %s\n", d, i, this.demes[d][i]);
    }
  }
  }
  ...
  for(int d = 0; d< this.demes.size(); ++d) {
    FredUtils.Status(0, "Loading population for Deme %d:\n", d);
  assert(this.demes[d].size() > 0);
    for(int i = 0; i< this.demes[d].size(); ++i) {
      FredUtils.Status(0, "Loading population %d for Deme %d: %s\n", i, d, this.demes[d][i]);
  ...
  read_population(pop_dir, this.demes[d][i], "people");
      if(Global.Enable_Group_Quarters) {
        FredUtils.Status(0, "Loading group quarters population %d for Deme %d: %s\n", i, d,
        this.demes[d][i]);
  // o---------------------------------------- Call read_population to actually
  // |                                         read the population files
  // V
  ...
  void read_population(string pop_dir, string pop_id, string pop_type)
  {

    FredUtils.Status(0, "read population entered\n");
  ...
  if(fp != NULL) {
    ...
    FredUtils.Log(0, "calling SnappyFileCompression on pop_file = %s\n", pop_file);
...
#pragma omp parallel
    ...
    FredUtils.Log(0, "finished reading compressed population, pop_size = %d\n", pop_size);
    return;
  }
#endif
    ...
    if (fp == NULL)
    {
      FredUtils.Abort("population_file %s not found\n", population_file);
    }
    fclose(fp);
    if (this.enable_copy_files)
    {
    ...
    FredUtils.Log(0, "finished reading uncompressed population, pop_size = %d\n", pop_size);
  }
  ...
  void remove_dead_person_from_population(int day, Person* person)
  {
    ...
    FredUtils.Log(1, "DELETING PERSON: %d ...\n", person.get_id());
    person.terminate(day);
    FredUtils.Log(1, "DELETED PERSON: %d\n", person.get_id());
  ...
  void report(int day)
  {
    ...
    if (output_population > 0)
    {
      int month;
      int day_of_month;
      sscanf(output_population_date_match, "%d-%d", &month, &day_of_month);
      if ((day == 0)
  ...
  void quality_control()
  {
    if (Global.Verbose > 0)
    {
      Console.WriteLine("population quality control check\n");
    ...
    for (int p = 0; p < this.get_index_size(); ++p)
    {
      ...
      {
        Console.WriteLine("Help! Person %d has no home.\n", person.get_id());
    ...
    if (Global.Verbose > 0)
    {
      ...
      for (int c = 0; c < 20; ++c)
      {
        Console.WriteLine("age %2d to %d: %6d (%.2f%%)\n", 5 * c, 5 * (c + 1) - 1, count[c],
          (100.0 * count[c]) / total);
      }
      Console.WriteLine("AGE 0-4: %d %.2f%%\n", n0, (100.0 * n0) / total);
      Console.WriteLine("AGE 5-17: %d %.2f%%\n", n5, (100.0 * n5) / total);
      Console.WriteLine("AGE 18-64: %d %.2f%%\n", n18, (100.0 * n18) / total);
      Console.WriteLine("AGE 65-100: %d %.2f%%\n", n65, (100.0 * n65) / total);
      Console.WriteLine("\n");
      ...
      if (Global.Enable_Vaccination)
      {
        for (int d = 0; d < Global.Diseases.Count; ++d)
        {
          if (Global.Diseases.get_disease(d).get_at_risk().is_empty() == false)
          {
            ...
            for (int c = 0; c < 10; ++c)
            {
              Console.WriteLine("age %2d to %2d: %6d (%.2f%%)\n", 10 * c, 10 * (c + 1) - 1,
    ...
    FredUtils.Status(0, "population quality control finished\n");
  }
  ...
  void assign_classrooms()
  {
    if (Global.Verbose > 0)
    {
      Console.WriteLine("assign classrooms entered\n");
    ...
    FredUtils.Log(0, "assign classrooms finished\n");
  }
  ...
  void assign_offices()
  {
    if (Global.Verbose > 0)
    {
      Console.WriteLine("assign offices entered\n");
    ...
    FredUtils.Log(0, "assign offices finished\n");
  }
  ...
  void assign_primary_healthcare_facilities()
  {
    ...
    if (Global.Verbose > 0)
    {
      Console.WriteLine("assign primary healthcare entered\n");
    ...
    FredUtils.Log(0, "assign primary healthcare finished\n");
  }

  void get_network_stats(string directory)
  {
    if (Global.Verbose > 0)
    {
      Console.WriteLine("get_network_stats entered\n");
    ...
    if (Global.Verbose > 0)
    {
      Console.WriteLine("get_network_stats finished\n");
  ...
  void set_school_income_levels()
  {
        ...
        itr != school_income_hh_mm.end(); ++itr)
    {
      ...
      FredUtils.Status(0, "MEAN_HH_INCOME: %s %.2f\n", schl.get_label(),
  ...
  void report_mean_hh_distance_from_school()
  {
        ...
        itr != school_enrollment_map.end(); ++itr)
    {
      ...
      FredUtils.Status(0, "MEAN_HH_DISTANCE: %s %.2f\n", (*itr).first.get_label(),
  ...
  void print_age_distribution(string dir, string date_string, int run)
  {
    FILE* fp;
  ...
  void write_population_output_file(int day)
  {
    ...
    sprintf(population_output_file, "%s/%s_%s.txt", Global.Output_directory, pop_outfile,
      Date::get_date_string().c_str());
    FILE* fp = fopen(population_output_file, "w");
    if (fp == NULL)
    {
      FredUtils.Abort("Help! population_output_file %s not found\n", population_output_file);
    }
  ...
  void set_health_insurance(Person* p)
  {
    ...
    //    if(hh_income <= low_income_threshold && FredRandom.NextDouble() < (1.0 - (hh_income / low_income_threshold))) {
