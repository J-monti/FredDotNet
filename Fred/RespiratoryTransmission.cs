...
namespace Fred
{
  public class RespiratoryTransmission : Transmission
  {
    public RespiratoryTransmission()
    {
    ...
    void setup(Disease* disease)
    {
      ...
      /*
      prob_contact = new double * [101];
      // create a PolyMod type matrix;
      for(int i = 0; i <= 100; ++i) {
        prob_contact[i] = new double [101];

        for(int j = 0; j <= 100; ++j) {
          prob_contact[i][j] = 0.05;
        }
      }
      for(int i = 0; i <= 100; ++i) {
      ...
      continue;
          ...
          prob_contact[i][j] = 1.0 - 0.2 * abs(i - j);
        }
      }
      */
    }
    ...
    bool attempt_transmission(double transmission_prob, Person* infector, Person* infectee,
