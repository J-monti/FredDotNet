...
namespace Fred
{
  public class Travel
  {
    ...
    void read_hub_file()
    {
      ...
      if (fp == NULL)
      {
        FredUtils.Abort("Help! Can't open travel_hub_file %s\n", hub_file);
      }
      if (Global.Verbose > 0)
      {
        Console.WriteLine("reading travel_hub_file %s\n", hub_file);
        fflush(stdout);
      }
      hubs.clear();
      hub_t hub;
    ...
    void read_trips_per_day_file()
    {
      ...
      if (fp == NULL)
      {
        FredUtils.Abort("Help! Can't open trips_per_day_file %s\n", trips_per_day_file);
      }
      if (Global.Verbose > 0)
      {
        Console.WriteLine("reading trips_per_day_file %s\n", trips_per_day_file);
        fflush(stdout);
      }
      for (int i = 0; i < num_hubs; ++i)
      {
        for (int j = 0; j < num_hubs; ++j)
        {
          ...
          if (fscanf(fp, "%d ", &n) != 1)
          {
            FredUtils.Abort("ERROR: read failed on file %s", trips_per_day_file);
          }
          trips_per_day[i][j] = n;
        }
      }
      ...
      if (Global.Verbose > 1)
      {
        ...
        fflush(stdout);
        Console.WriteLine("finished reading trips_per_day_file %s\n", trips_per_day_file);
      }
    }
    ...
    void setup_travelers_per_hub()
    {
      ...
      FredUtils.Log(0, "Preparing to set households: %li \n", households);
      for (int i = 0; i < households; ++i)
      {
        ...
        FredUtils.Log(2, "h_id: %li h_county: %i \n", h_id, h_county);
        ...
        if (closest > -1)
        {
          FredUtils.Log(1, "h_id: %li from county: %i  assigned to the airport: %i, distance:  %f\n", h_id, h_county, hubs[closest].id, min_dist);
    ...
    void update_travel(int day)
    {
      ...
      if (Global.Verbose > 0)
      {
        Console.WriteLine("update_travel entered day %d\n", day);
      ...
      for (int i = 0; i < num_hubs; ++i)
      {
        ...
        for (int j = 0; j < num_hubs; ++j)
        {
          ...
          FredUtils.Log(1, "TRIPCOUNT day %d i %d j %d count %d\n", day, i, j, count);
          for (int t = 0; t < count; ++t)
          {
               ...
               host != NULL && (!host.get_travel_status()))
            {
              ...
              if (traveler.get_travel_status())
              {
                ...
                FredUtils.Status(1, "RETURN_FROM_TRAVEL EVENT ADDED today %d duration %d returns %d id %d age %d\n",
                day, duration, return_sim_day, traveler.get_id(), traveler.get_age());
                successful_trips++;
              }
            }
          }
          FredUtils.Log(1, "DAY %d SRC = %d DEST = %d TRIPS = %d\n", day, hubs[i].id, hubs[j].id, successful_trips);
        }
      }
      ...
      if (Global.Verbose > 1)
      {
        Console.WriteLine("update_travel finished\n");
    ...
    void find_returning_travelers(int day)
    {
      ...
      for (int i = 0; i < size; i++)
      {
        ...
        FredUtils.Status(1, "RETURNING FROM TRAVEL today %d id %d age %d\n",
    ...
    void quality_control(string directory)
    {
    }
